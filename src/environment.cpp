...
Environment::Environment(SymbolTable* globalScope)
{
	scopeDepth = 0;
	this->globalScope = globalScope;
	this->globalScope->setSlotOffset(0);
	scopes.emplace_back(this->globalScope);	// emplace the 'global' scope, which is always the scope at index 0 in the scopes vector
}

void Environment::pushScope(SymbolTable* table, ScopeKind kind)
{
	...
	table->setSlotOffset(parentScopeSlotOffset);
	scopes.emplace_back(table);
	scopeDepth++;
}
