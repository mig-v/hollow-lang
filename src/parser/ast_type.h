...
{
public:
	ASTArrayType(ASTType* elementType, ASTExpr* size, uint8_t dimensionCount);
	bool operator==(const ASTType& other) const override;
...
{
public:
	ASTStructType(Token identifier);
	bool operator==(const ASTType& other) const override;
...
{
public:
	ASTPointerType(ASTType* baseType, uint8_t indirectionLevel);
	bool operator==(const ASTType& other) const override;

	ASTType* baseType;
	uint8_t indirectionLevel;
};