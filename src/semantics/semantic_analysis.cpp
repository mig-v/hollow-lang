...
void SemanticAnalysis::analyze(std::vector<ASTNode*>& ast, MemoryArena* typeArena, DiagnosticReporter* diagnosticReporter, Environment* env, TypeTable* typeTable)
{
...
void SemanticAnalysis::visitVarDecl(ASTVarDecl& node)
{
	...
	// make sure the variable isnt a void base type -> x : void, x : void[], x : void*
	if (SemanticUtils::typeIsVoid(node.typeInfo))
		diagnosticReporter->reportDiagnostic("cannot declare variable of base type void", DiagnosticLevel::Error, DiagnosticType::IncompatibleType, ErrorPhase::Semantic, node.line, node.col);

	if (node.typeInfo->type == TypeKind::Array)
		node.typeInfo->elementSize = typeTable->getSize(node.typeInfo->elementType);
}
...
void SemanticAnalysis::visitFieldAccess(ASTFieldAccess& node)
{
	node.typeInfo = typeArena->alloc<TypeInfo>();
	node.structExpr->accept(*this);

	TypeInfo* currType = node.structExpr->typeInfo;

	while (currType->type == TypeKind::Pointer)
	{
		currType = currType->ptrBaseType;
	}

	if (!currType || currType->type != TypeKind::Struct)
	{
		diagnosticReporter->reportDiagnostic("cannot access field of non struct type", DiagnosticLevel::Error, DiagnosticType::InvalidFieldName, ErrorPhase::Semantic, node.line, node.col);
		node.typeInfo->type = TypeKind::Unknown;
		return;
	}

	const std::string& fieldIdentifier = std::get<std::string>(node.fieldIdentifier.value);

	if (!typeTable->typeHasField(node.structExpr->typeInfo->typeName, fieldIdentifier))
	{
		std::string message = "type " + node.structExpr->typeInfo->typeName + " has no field " + fieldIdentifier;
		diagnosticReporter->reportDiagnostic(message, DiagnosticLevel::Error, DiagnosticType::InvalidFieldName, ErrorPhase::Semantic, node.line, node.col);
		node.typeInfo->type = TypeKind::Unknown;
		return;
	}

	*(node.typeInfo) = *(typeTable->lookupType(node.structExpr->typeInfo->typeName)->structFields[fieldIdentifier].typeInfo);
}

void SemanticAnalysis::visitStructDecl(ASTStructDecl& node)
{
	TypeEntry* type = typeTable->lookupType(std::get<std::string>(node.type->identifier.value));

	// disallow struct fields of type void
	for (size_t i = 0; i < type->fieldOrder.size(); i++)
	{
		if (SemanticUtils::typeIsVoid(type->fieldOrder[i]->typeInfo))
			diagnosticReporter->reportDiagnostic("cannot declare struct field of base type void", DiagnosticLevel::Error, DiagnosticType::IncompatibleType, ErrorPhase::Semantic, node.fields[i]->line, node.fields[i]->col);
	}
}