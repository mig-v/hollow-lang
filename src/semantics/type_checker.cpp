...
bool TypeChecker::pointerAssignmentIsLegal(TypeInfo* lhs, TypeInfo* rhs)
{
	if (lhs->type != TypeKind::Pointer || rhs->type != TypeKind::Pointer)
		return false;

	// if both sides are pointers, verify they have same level of indirection and same basePtrType
	// if they have different indirection levels, then they arent compatible
	if (lhs->indirectionLevel != rhs->indirectionLevel)
		return false;

	// if they are the same indirection level, we need to check if they have the same base type
	TypeInfo* currLhs = lhs;
	TypeInfo* currRhs = rhs;

	for (uint8_t i = 0; i < lhs->indirectionLevel; i++)
	{
		currLhs = currLhs->ptrBaseType;
		currRhs = currRhs->ptrBaseType;
	}

	if (currLhs->type == TypeKind::Struct && currRhs->type == TypeKind::Struct)
		return currLhs->typeName == currRhs->typeName;
	else if (currLhs->type == TypeKind::Array && currRhs->type == TypeKind::Array)
		return false;
	else
		return currLhs->type == currRhs->type;

	return false;
}

void TypeChecker::typeCheck(std::vector<ASTNode*>& ast, MemoryArena* nodeArena, MemoryArena* typeArena, DiagnosticReporter* diagnosticReporter, TypeTable* typeTable)
{
...
void TypeChecker::visitAssign(ASTAssign& node)
{
	...
	{
		if (node.value->typeInfo->type == TypeKind::Struct && node.assignee->typeInfo->type == TypeKind::Struct)
		{
			if (node.value->typeInfo->typeName != node.assignee->typeInfo->typeName)
			{
				std::string message = "cannot assign " + node.value->typeInfo->typeName + " to " + node.assignee->typeInfo->typeName;
				diagnosticReporter->reportDiagnostic(message, DiagnosticLevel::Error, DiagnosticType::IncompatibleType, ErrorPhase::TypeChecker, node.line, node.col);
				return;
			}
		}

		if (node.value->typeInfo->type == TypeKind::Array && node.assignee->typeInfo->type == TypeKind::Array)
		{
			diagnosticReporter->reportDiagnostic("cannot assign arrays directly", DiagnosticLevel::Error, DiagnosticType::IncompatibleType, ErrorPhase::TypeChecker, node.line, node.col);
			return;
		}

		if (node.value->typeInfo->type == TypeKind::Pointer || node.assignee->typeInfo->type == TypeKind::Pointer)
		{
			if (!pointerAssignmentIsLegal(node.assignee->typeInfo, node.value->typeInfo))
			{
				diagnosticReporter->reportDiagnostic("incompatible pointer types for assignment", DiagnosticLevel::Error, DiagnosticType::IncompatibleType, ErrorPhase::TypeChecker, node.line, node.col);
				return;
			}
		}
	...
	{
		diagnosticReporter->reportDiagnostic("incompatible operands for assignment", DiagnosticLevel::Error, DiagnosticType::IncompatibleOperands, ErrorPhase::TypeChecker, node.line, node.col);
	}
}
...
void TypeChecker::visitUnaryExpr(ASTUnaryExpr& node)
{
	...
	else if (node.op.type == TokenType::BitwiseAnd)
	{
		...
		{
			diagnosticReporter->reportDiagnostic("cannot take address of non l-value type", DiagnosticLevel::Error, DiagnosticType::IncompatibleOperands, ErrorPhase::TypeChecker, node.line, node.col);
			node.typeInfo->type = TypeKind::Unknown;
			return;
		}

		TypeInfo* curr = node.expr->typeInfo;
		node.typeInfo->type = TypeKind::Pointer;
		node.typeInfo->ptrBaseType = curr;
		node.typeInfo->indirectionLevel = node.expr->typeInfo->indirectionLevel + 1;
	}

	// pointer deref -> x = *y;
	else if (node.op.type == TokenType::Asterisk)
	{
		if (node.expr->typeInfo->type != TypeKind::Pointer)
		{
			diagnosticReporter->reportDiagnostic("cannot dereference non pointer type", DiagnosticLevel::Error, DiagnosticType::IncompatibleOperands, ErrorPhase::TypeChecker, node.line, node.col);
			node.typeInfo->type = TypeKind::Unknown;
			return;
		}

		node.typeInfo = node.expr->typeInfo->ptrBaseType;
		node.typeInfo->indirectionLevel = node.expr->typeInfo->indirectionLevel - 1;
	}
	else
	{
...
{
	if (SemanticUtils::typeIsVoid(node.typeInfo))
		diagnosticReporter->reportDiagnostic("cannot have parameter of base type void", DiagnosticLevel::Error, DiagnosticType::IncompatibleType, ErrorPhase::TypeChecker, node.line, node.col);
}
